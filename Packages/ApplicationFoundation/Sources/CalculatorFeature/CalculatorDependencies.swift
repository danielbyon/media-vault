...
public extension DependencyValues {
  ...
  var calculatorClipboard: CalculatorClipboardClient {
...
public extension DependencyValues {
  ...
  var calculatorPersistence: CalculatorPersistenceClient {
...
private actor CalculatorDatabase {
  ...
  static func live() -> CalculatorDatabase {
    do {
      ...
      let database = try PersistenceStore.makePersistent(
        at: calculatorDirectory.appendingPathComponent("calculator.sqlite")
      )
      return CalculatorDatabase(database: database)
    } catch {
  ...
  func load() throws -> CalculatorSnapshot? {
    guard let database else { throw CalculatorPersistenceError.unavailable }
