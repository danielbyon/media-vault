...
public struct CalculatorEngine: Sendable {
  ...
  private struct Parser {
    ...
    mutating func parse() throws -> Decimal {
      skipWhitespace()
      guard !isAtEnd else { throw CalculatorError.invalidExpression }
    ...
    private mutating func parseExpression() throws -> Decimal {
      ...
      while true {
        skipWhitespace()
        if consume("+") {
          value = try rounded(adding: value, try parseTerm())
        } else if consume("−") || consume("-") {
          value = try rounded(subtracting: value, try parseTerm())
        } else {
          return value
        }
      }
    ...
    private mutating func parseTerm() throws -> Decimal {
      ...
      while true {
        skipWhitespace()
        if consume("×") || consume("*") {
          value = try rounded(multiplying: value, try parsePostfix())
        } else if consume("÷") || consume("/") {
          value = try rounded(dividing: value, try parsePostfix())
        } else {
          return value
        }
      }
    ...
    private mutating func parsePrimary() throws -> Decimal {
      ...
      if consume("−") || consume("-") {
        return try rounded(subtracting: 0, try parsePrimary())
      }
      if consume("(") {
        let value = try parseExpression()
        skipWhitespace()
    ...
    private mutating func parseNumber() throws -> Decimal {
      ...
      guard index > start else { throw CalculatorError.invalidExpression }
    ...
    private func rounded(
      ...
      guard error == .noError else { throw CalculatorError.overflow }
